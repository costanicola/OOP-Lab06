...
public class StrictBankAccount implements BankAccount {
    ...
    public void depositFromATM(final int usrID, final double amount) {
        if (transactionsOnATM < nMaxATMTransactions) {
    ...
    public void computeManagementFees(final int usrID) {
        final double feeAmount = MANAGEMENT_FEE + (totalTransactions * StrictBankAccount.TRANSACTION_FEE);
        if (checkUser(usrID) && isWithdrawAllowed(feeAmount)) {
    		 balance -= MANAGEMENT_FEE + totalTransactions * StrictBankAccount.TRANSACTION_FEE;
             totalTransactions = 0;
             transactionsOnATM = 0;
        }
    }
